...
bool askUser(string question) {
	...
	do {
		...
		} else if (val == "YES" || val == "yes" || val == "Yes" || val == "Y" || val == "y") {
			ret=true;
		} else if(val == "NO" || val == "no" || val == "No" || val == "N" || val == "n") {
			ret=false;
		} else {
			valid = false;
			cin.clear();				// clears state of error of the buffer
...
vector<Road> getBlockedRoads() {
	...
	while (!file.eof()) {
		...
		if (file.eof()) {
			vector<Road> tmp;
			cout << "Warning: file 'BlockedRoads.txt' is empty or incomplete"
					<< endl;
			return tmp;
		}
		getline(file, str[1]);
...
void ecopontosLoad() {
	...
	do {
		...
		cout << endl
				<< "Please insert minimum load of the ecopontos in kg (0 to 100): ";
		cin >> min_load;
		cin.ignore(1000, '\n');
	...
	for (list<Ecoponto>::iterator it = eco.begin(); it != eco.end(); it++) // for every ecoponto
		...
		for (unsigned int i = 0; i <= connected.size(); i++) // for every node that we can access from initial
			...
			if (i == connected.size()) { 						// if not found
				...
				cout
						<< "Connectity error: Ecoponto " + id
								+ " cannot be reached!" << endl;
				getchar();
				return false;
			}
		}

		for (list<Ecoponto>::iterator ita = eco.begin(); ita != eco.end();
				ita++) {
			if (it != ita) {
				vector<Coord> connected2 = gr->bfs(
						gr->getVertex((*it).getLocation()));
				gr->resetVisited();
				...
				for (unsigned int i = 0; i <= connected2.size(); i++) // for every node that we can access from the current ecoponto
					...
					if (i == connected2.size()) { 				// if not found
						...
						cout
								<< "Connectity error: Ecoponto " + id_d
										+ " cannot be reached from " + id_s
										+ "!" << endl;
						getchar();
						return false;
					}
...
Truck popBestTruck(list<Truck> &trucks, int totalTrash) {
	int bestCapacity = 0;
	Truck best;
	for (list<Truck>::iterator it = trucks.begin();
			bestCapacity < totalTrash && it != trucks.end(); ++it) {
		best = (*it);
...
int totalTrash(list<Ecoponto> ecopontos) {
	int res = 0;
	for (list<Ecoponto>::iterator it = ecopontos.begin(); it != ecopontos.end();
			++it)
		res += (*it).getTrash();
	return res;
}
...
list<Ecoponto> fillMax(list<Ecoponto> &ecopontos, Truck truck) {
	...
	for (list<Ecoponto>::reverse_iterator it = ecopontos.rbegin();
			it != ecopontos.rend() && currentCapacity > 0;) {
